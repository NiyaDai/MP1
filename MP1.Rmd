...
diamonds_plot <- ggplot(data=diamonds, aes(x=carat, y=price)) +
  ...
  labs(title="Prices of over 50,000 round cut diamonds",
       subtitle = "Effect of clarity on price of diamonds at each weight. Clarity (from worst to best) is represented by different colors.",
...
diamonds_plot <- ggplot(data=diamonds, aes(x=carat, y=price, color = cut)) +
  scale_color_brewer(palette="PuBuGn")+
  labs(title="Prices of over 50,000 round cut diamonds",
       subtitle = "Effect of color on price of diamonds at each weight. According to jewelry retailer Blue Nile, the color grade is arranged from Colorless (D, E, and F) to Near-Colorless (G, H, I, J). Near-Colorless diamonds contain small traces of color, but can’t be seen very easily with the naked eye.",
